...
it.effect("accepts provider-scoped startup options in thread.turn.start", () =>
	Effect.gen(function* () {
		...
		assert.strictEqual(
			parsed.providerOptions?.gemini?.binaryPath,
			"/usr/local/bin/gemini",
		);
		assert.strictEqual(
			parsed.providerOptions?.gemini?.homePath,
			"/tmp/.gemini",
		);
	}),
);
...
it.effect(
	"accepts provider-scoped startup options in thread.turn-start-requested payload",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeThreadTurnStartRequestedPayload({
				threadId: "thread-1",
