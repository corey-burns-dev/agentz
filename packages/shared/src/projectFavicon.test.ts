...
describe("projectFavicon helpers", () => {
	it("normalizes slash variants into clean relative paths", () => {
		expect(normalizeProjectRelativePath("./public\\favicon.svg")).toBe(
			"public/favicon.svg",
		);
	});
	...
	it("flags common favicon-style basenames as likely candidates", () => {
		...
		expect(isLikelyProjectFaviconPath("src/app/apple-touch-icon.png")).toBe(
			true,
		);
	...
	it("sorts favicon-like paths ahead of generic logos", () => {
		expect(
			["assets/logo.png", "public/favicon.svg"].toSorted(
				compareProjectFaviconPaths,
			),
		).toEqual(["public/favicon.svg", "assets/logo.png"]);
	});
});
