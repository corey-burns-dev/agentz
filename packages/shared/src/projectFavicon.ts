const PROJECT_FAVICON_EXTENSIONS = new Set([".ico", ".png", ".svg", ".jpg", ".jpeg", ".webp"]);
...
function basenameWithoutExtension(input: string): string {
  ...
  return extension.length === 0 ? basename : basename.slice(0, basename.length - extension.length);
}
...
export function normalizeProjectRelativePath(input: string): string {
  return toPosixPath(input).replace(STRIP_LEADING_PATH_RE, "").replaceAll(/\/+/g, "/").trim();
}
...
export function isLikelyProjectFaviconPath(input: string): boolean {
  ...
  const basename = basenameWithoutExtension(normalizeProjectRelativePath(input).toLowerCase());
  return (
    basename === "favicon" ||
...
export function getProjectFaviconPathScore(input: string): number {
  ...
  if (basename === "favicon") {
    score = 0;
  } else if (basename.startsWith("favicon-") || basename.startsWith("favicon_")) {
    score = 1;
    ...
    score = 4;
  } else if (basename === "icon" || basename.startsWith("icon-") || basename.startsWith("icon_")) {
    score = 5;
  } else if (basename.includes("icon")) {
    score = 6;
  } else if (basename === "logo" || basename.startsWith("logo-") || basename.startsWith("logo_")) {
    score = 7;
...
export function compareProjectFaviconPaths(left: string, right: string): number {
  const scoreDelta = getProjectFaviconPathScore(left) - getProjectFaviconPathScore(right);
