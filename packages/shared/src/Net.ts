...
/**
 * Returns a settle function that forwards an Effect to `resume` exactly once.
 * Subsequent calls are no-ops.
 */
function makeSettle<A, E = never>(
  resume: (effect: Effect.Effect<A, E>) => void,
): (effect: Effect.Effect<A, E>) => void {
  let settled = false;
  return (effect) => {
    if (settled) return;
    settled = true;
...
/**
 * Try to reserve a specific port on any interface.
 * Returns the port number on success, fails with NetError on conflict.
 */
const tryReservePort = (port: number): Effect.Effect<number, NetError> =>
  Effect.callback<number, NetError>((resume) => {
    const server = Net.createServer();
    const settle = makeSettle(resume);
  ...
  static readonly layer = Layer.sync(NetService, () => {
    ...
    const canListenOnHost = (port: number, host: string): Effect.Effect<boolean> =>
      Effect.callback<boolean>((resume) => {
        ...
        server.once("error", (cause) => {
          settle(Effect.succeed(isErrnoExceptionWithCode(cause) && cause.code === "EADDRNOTAVAIL"));
        });
