import type { OrchestrationEvent, ProjectionThreadTurnStatus, ThreadId } from "@agents/contracts";
...
function readForegroundState(threadIsActive: boolean): {
  ...
  const isForegroundForThread = isWindowVisible && isWindowFocused && threadIsActive;
...
function getThreadContextFromStore(
  ...
  const project = state.projects.find((entry) => entry.id === thread.projectId) ?? null;
...
function showTurnCompletedToast(context: ThreadContext): void {
  ...
  const description =
    parts.length > 0 ? parts.join(" \u00b7 ") : "Agent turn completed in background.";
...
function handleTurnDiffCompleted(
  ...
  const perProject = getProjectNotificationSettingsForKey(threadContext.projectKey);

  const isActiveThread =
    context.activeThreadId !== null && context.activeThreadId === threadContext.threadId;
...
function handleActivityAppended(
  ...
  const threadContext = getThreadContextFromStore(payload.threadId, "error", null);
  const perProject = getProjectNotificationSettingsForKey(threadContext.projectKey);
  if (!shouldNotifyForError(threadContext, perProject)) {
    return;
  }
