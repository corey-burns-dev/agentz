...
/**
 * Applies a mutated draft to state, removing it entirely when it becomes empty.
 * Returns a partial state update, or null if nothing changed.
 */
...
/**
 * Updates a single scalar field on a thread's composer draft.
 * Returns null (no-op) when the value is already equal to the current value.
 */
function applyDraftFieldUpdate<K extends keyof ComposerThreadDraftState>(
  state: Pick<ComposerDraftStoreState, "draftsByThreadId">,
  threadId: ThreadId,
  field: K,
  value: ComposerThreadDraftState[K],
): Pick<ComposerDraftStoreState, "draftsByThreadId"> | null {
  const base = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
  if (base[field] === value) return null;
  return applyDraftUpdate(state, threadId, { ...base, [field]: value });
}
  ...
  persist(
    (set, get) => ({
      ...
      setPrompt: (threadId, prompt) => {
        ...
        set((state) => applyDraftFieldUpdate(state, threadId, "prompt", prompt) ?? state);
      },
      setProvider: (threadId, provider) => {
        if (threadId.length === 0) return;
        set(
          (state) =>
            applyDraftFieldUpdate(state, threadId, "provider", normalizeProviderKind(provider)) ??
            state,
        );
      },
      setModel: (threadId, model) => {
        ...
        set(
          (state) =>
            applyDraftFieldUpdate(state, threadId, "model", normalizeModelSlug(model) ?? null) ??
            state,
        );
      },
      setRuntimeMode: (threadId, runtimeMode) => {
        ...
        set((state) => applyDraftFieldUpdate(state, threadId, "runtimeMode", value) ?? state);
      },
      setInteractionMode: (threadId, interactionMode) => {
        if (threadId.length === 0) return;
        const value =
          interactionMode === "plan" || interactionMode === "default" ? interactionMode : null;
        set((state) => applyDraftFieldUpdate(state, threadId, "interactionMode", value) ?? state);
      },
      setEffort: (threadId, effort) => {
        ...
        const value =
          ...
          effort !== DEFAULT_REASONING_EFFORT_BY_PROVIDER.codex
            ? effort
            : null;
        set((state) => applyDraftFieldUpdate(state, threadId, "effort", value) ?? state);
      },
      setCodexFastMode: (threadId, enabled) => {
        if (threadId.length === 0) return;
        set(
          (state) =>
            applyDraftFieldUpdate(state, threadId, "codexFastMode", enabled === true) ?? state,
        );
      },
      ...
      removeImage: (threadId, imageId) => {
        ...
        set((state) => {
          ...
          return applyDraftUpdate(state, threadId, {
            ...current,
            images: current.images.filter((image) => image.id !== imageId),
            nonPersistedImageIds: current.nonPersistedImageIds.filter((id) => id !== imageId),
            persistedAttachments: current.persistedAttachments.filter((a) => a.id !== imageId),
          });
        });
      },
      ...
      syncPersistedAttachments: (threadId, attachments) => {
        ...
        Promise.resolve().then(() => {
          ...
          set((state) => {
            ...
            return applyDraftUpdate(state, threadId, {
              ...current,
              persistedAttachments: attachments.filter(
                (a) => imageIdSet.has(a.id) && persistedIdSet.has(a.id),
              ),
              nonPersistedImageIds: current.images
                .map((image) => image.id)
    ...
    }),
    {
      ...
      version: 1,
      storage: createJSONStorage(() => composerDraftDebouncedStorage ?? localStorage),
      partialize: (state) => {
