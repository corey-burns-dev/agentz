...
describe("getVscodeIconUrlForEntry", () => {
	it("uses exact filename matches from the vscode-icons manifest", () => {
		const darkUrl = getVscodeIconUrlForEntry(
			"pnpm-workspace.yaml",
			"file",
			"dark",
		);
		const lightUrl = getVscodeIconUrlForEntry(
			"pnpm-workspace.yaml",
			"file",
			"light",
		);
	...
	it("uses longest extension match for compound extensions", () => {
		...
		const manifestStyleUrl = getVscodeIconUrlForEntry(
			"buf.yaml",
			"file",
			"dark",
		);
	...
	it("uses folder mappings and light-aware filename mappings", () => {
		const folderUrl = getVscodeIconUrlForEntry(
			"packages/src",
			"directory",
			"light",
		);
	...
	it("falls back to language-based mappings for path-only cases", () => {
		...
		const dockerfileUrl = getVscodeIconUrlForEntry(
			"Dockerfile",
			"file",
			"dark",
		);
		...
		const cursorRulesUrl = getVscodeIconUrlForEntry(
			"general.mdc",
			"file",
			"dark",
		);
		const githubWorkflowUrl = getVscodeIconUrlForEntry(
