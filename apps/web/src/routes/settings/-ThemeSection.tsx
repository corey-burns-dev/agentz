...
const PROJECT_FAVICON_SIZE_LABELS: Record<(typeof PROJECT_FAVICON_SIZE_OPTIONS)[number], string> = {
  small: "Small",
  medium: "Medium",
  large: "Large",
};

const SIDEBAR_SPACING_LABELS: Record<(typeof SIDEBAR_SPACING_OPTIONS)[number], string> = {
  compact: "Compact",
  default: "Default",
  spacious: "Spacious",
};

const THEME_MODE_OPTIONS_LIST: ReadonlyArray<SettingsOption<ThemeMode>> = THEME_MODE_OPTIONS.map(
  (mode) => ({
    value: mode,
...
const DENSITY_OPTIONS_LIST: ReadonlyArray<SettingsOption<(typeof DENSITY_OPTIONS)[number]>> =
  DENSITY_OPTIONS.map((density) => ({
    value: density,
    label: DENSITY_LABELS[density],
  }));

const RADIUS_OPTIONS_LIST: ReadonlyArray<SettingsOption<(typeof RADIUS_PRESETS)[number]>> =
...
const PALETTE_OPTIONS: ReadonlyArray<SettingsOption<(typeof DARK_THEME_PRESETS)[number]>> =
...
function RadiusPreviewCard({
  ...
  return (
    <div className="flex w-full flex-col items-center gap-2">
      ...
      />
      <span
        className={`text-2xs font-medium ${selected ? "text-primary" : "text-muted-foreground"}`}
      >
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2.5">
          <div className="space-y-1">
            ...
            <SettingsHint>
              Light mode uses a single shared palette. Dark mode exposes additional presets.
            </SettingsHint>
          </div>
          {resolvedTheme === "light" ? (
        ...
        <div className="space-y-2.5">
          <div className="space-y-1">
            <p className="text-xs font-medium text-foreground">Project favicon size</p>
            <SettingsHint>
              Controls the favicon size shown beside each project in the sidebar.
            </SettingsHint>
          </div>
          <SettingsSegmentedControl
            legend="Project favicon size"
            value={uiSettings.projectFaviconSize}
            onChange={(projectFaviconSize) => updateUISettings({ projectFaviconSize })}
        ...
        <div className="space-y-2.5">
          <div className="space-y-1">
            <p className="text-xs font-medium text-foreground">Sidebar row spacing</p>
            <SettingsHint>
              Controls the line height and gap between project and thread rows in the sidebar.
            </SettingsHint>
          </div>
          <SettingsSegmentedControl
        ...
        <div className="space-y-2.5">
          <div className="space-y-1">
            ...
            <SettingsHint>
              Use sharper chrome for dense workspaces or softer corners for a more relaxed look.
            </SettingsHint>
          </div>
          <SettingsCardRadioGroup
