...
describe("isCollapsedCursorAdjacentToMention", () => {
	it("returns false when no mention exists", () => {
		expect(isCollapsedCursorAdjacentToMention("plain text", 6, "left")).toBe(
			false,
		);
		expect(isCollapsedCursorAdjacentToMention("plain text", 6, "right")).toBe(
			false,
		);
	});
	...
	it("keeps @query typing non-adjacent while no mention pill exists", () => {
		...
		expect(isCollapsedCursorAdjacentToMention(text, text.length, "left")).toBe(
			false,
		);
		expect(isCollapsedCursorAdjacentToMention(text, text.length, "right")).toBe(
			false,
		);
	});
	...
	it("detects left adjacency only when cursor is directly after a mention", () => {
		...
		expect(isCollapsedCursorAdjacentToMention(text, mentionEnd, "left")).toBe(
			true,
		);
		expect(isCollapsedCursorAdjacentToMention(text, mentionStart, "left")).toBe(
			false,
		);
		expect(
			isCollapsedCursorAdjacentToMention(text, mentionEnd + 1, "left"),
		).toBe(false);
	});
	...
	it("detects right adjacency only when cursor is directly before a mention", () => {
		...
		expect(
			isCollapsedCursorAdjacentToMention(text, mentionStart, "right"),
		).toBe(true);
		expect(isCollapsedCursorAdjacentToMention(text, mentionEnd, "right")).toBe(
			false,
		);
		expect(
			isCollapsedCursorAdjacentToMention(text, mentionStart - 1, "right"),
		).toBe(false);
	});
});
