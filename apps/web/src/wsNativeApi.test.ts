...
const subscribeMock = vi.fn<
	(channel: string, listener: (data: unknown) => void) => () => void
>((channel, listener) => {
	const listeners =
		channelListeners.get(channel) ?? new Set<(data: unknown) => void>();
...
function getWindowForTest(): Window &
	typeof globalThis & { desktopBridge?: unknown } {
...
describe("wsNativeApi", () => {
	it("delivers and caches valid server.welcome payloads", async () => {
		...
		const { createWsNativeApi, onServerWelcome } = await import(
			"./wsNativeApi"
		);
	...
	it("delivers and caches valid server.configUpdated payloads", async () => {
		const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
		const { createWsNativeApi, onServerConfigUpdated } = await import(
			"./wsNativeApi"
		);
