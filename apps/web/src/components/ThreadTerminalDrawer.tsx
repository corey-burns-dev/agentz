...
function terminalThemeFromApp(): ITheme {
	...
	const background =
		...
		(isDark ? "rgb(14, 18, 24)" : "rgb(255, 255, 255)");
	const foreground =
...
function TerminalViewport({
	...
	useEffect(() => {
		...
		const fitTimer = window.setTimeout(() => {
			...
			if (!activeTerminal || !activeFitAddon) return;
			const wasAtBottom =
				activeTerminal.buffer.active.viewportY >=
				activeTerminal.buffer.active.baseY;
			activeFitAddon.fit();
			if (wasAtBottom) {
	...
	}, [cwd, runtimeEnv, terminalId, threadId, autoFocus]);
...
export default function ThreadTerminalDrawer({
	...
	const resolvedActiveTerminalId = normalizedTerminalIds.includes(
		activeTerminalId,
	)
		? activeTerminalId
	...
	const visibleTerminalIds = resolvedTerminalGroups[resolvedActiveGroupIndex]
		?.terminalIds ?? [resolvedActiveTerminalId];
	...
	const hasReachedTerminalLimit =
		normalizedTerminalIds.length >= MAX_THREAD_TERMINAL_COUNT;
							...
							>
								{visibleTerminalIds.map((terminalId) => (
									<div
										key={terminalId}
										role="tab"
										tabIndex={terminalId === resolvedActiveTerminalId ? 0 : -1}
										aria-selected={terminalId === resolvedActiveTerminalId}
