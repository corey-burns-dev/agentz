...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const [implementationProvider, setImplementationProvider] = useState<ProviderKind>("codex");
  ...
  const threadCanSwitchProvider = Boolean(
    hasThreadStarted && latestTurnSettled && sessionIdleOrStopped && hasNoPendingApprovalOrInput,
  );
  ...
  const implementationReasoningOptions = getReasoningEffortOptions(implementationProvider);
  ...
  const { attachmentPreviewHandoffByMessageId, handoffAttachmentPreviews } = useImageAttachments({
  ...
  const _hasReachedTerminalLimit = terminalState.terminalIds.length >= MAX_THREAD_TERMINAL_COUNT;
  ...
  const handleAppendToPrompt = useCallback(
    (text: string) => {
      ...
      const needSpace =
        current.length > 0 && !/\s$/.test(current) && trimmed.length > 0 && !/^\s/.test(trimmed);
  ...
  const { lastInvokedScriptByProjectId, runProjectScript, saveProjectScript, updateProjectScript } =
    useProjectScripts({
      threadId,
      activeThreadId,
  ...
  const onSend = async (
    ...
    if (isWorking && !queued && !activePendingProgress) {
      ...
      setPendingQueue((prev) => [...prev, { threadId, prompt, images: [...composerImages] }]);
    ...
    const standaloneSlashCommand =
      imagesForSend.length === 0 ? parseStandaloneComposerSlashCommand(trimmed) : null;
    ...
    const isFirstMessage = !isServerThread || activeThreadForSend.messages.length === 0;
    const baseBranchForWorktree =
      isFirstMessage && envMode === "worktree" && !activeThreadForSend.worktreePath
    ...
    await (async () => {
      ...
      await api.orchestration.dispatchCommand({
        ...
        ...(selectedModelOptionsForDispatch
          ...
          : {}),
        ...(selectedProviderStartOptions ? { providerOptions: selectedProviderStartOptions } : {}),
  ...
  const flushPendingQueuedSend = useEffectEvent((next: (typeof pendingQueue)[number]) => {
  ...
  const onHandOffInThread = useCallback(
    (params: { provider: ProviderKind; model: ModelSlug; planMarkdown: string }) => {
      if (!activeThread) return;
      ...
      setComposerTrigger(detectComposerTrigger(implementationText, implementationText.length));
      setPlanSidebarOpen(false);
      scheduleComposerFocus();
    },
    [
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <ComposerArea
            ...
            canAttachImages={selectedProvider === "codex" || selectedProvider === "gemini"}
        ...
        {planSidebarOpen ? (
          <PlanSidebar
            ...
            implementationModelForPicker={implementationModelForPickerWithCustomFallback}
            modelOptionsByProvider={modelOptionsByProvider}
            availableProviders={availableProviderOptions}
            selectedServiceTierSetting={selectedServiceTierSetting}
            onImplementationProviderModelChange={onImplementationProviderModelChange}
