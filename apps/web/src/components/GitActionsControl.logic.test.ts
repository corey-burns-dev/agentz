import type { GitStatusResult } from "@agentz/contracts";
...
describe("when: HEAD is detached and there are no local changes", () => {
	...
	it("buildMenuItems keeps commit, push, and PR disabled", () => {
		const items = buildMenuItems(
			status({ branch: null, hasWorkingTreeChanges: false }),
			false,
		);
		assert.deepEqual(items, [
			{
...
describe("summarizeGitResult", () => {
	...
	it("returns PR-focused toast for created PR action", () => {
		const result = summarizeGitResult({
			...
			pr: {
				...
				number: 42,
				title:
					"feat: ship github shortcuts and improve PR CTA in success toast",
			},
		});
	...
	it("truncates long description text", () => {
		...
		assert.deepEqual(result, {
			title: "Created PR #99",
			description:
				"feat: this title is intentionally extremely long so we can validate t...",
		});
	});
});
