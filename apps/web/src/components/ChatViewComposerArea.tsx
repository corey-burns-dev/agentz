...
import { getDefaultReasoningEffort, normalizeModelSlug } from "@agents/shared/model";
import {
  BotIcon,
  CheckIcon,
  ChevronDownIcon,
...
import { buildExpandedImagePreview, type ExpandedImagePreview } from "./ChatViewMessageList";
            ...
            >
              {shortcutKey !== null ? (
                <>
                  ...
                  <span className="sr-only">Press {shortcutKey} to select</span>
                </>
              ) : null}
...
export interface ComposerAreaProps {
  ...
  onPromptChange: (nextValue: string, nextCursor: number, cursorAdjacentToMention: boolean) => void;
...
export function ComposerArea(props: ComposerAreaProps) {
  const voiceSupported = props.isVoiceInputSupported === true && props.onAppendToPrompt != null;
  const voiceDisabled =
    !voiceSupported ||
    props.isConnecting ||
      ...
      >
        ...
        >
          {props.activePendingApproval ? (
            ...
            <div className="rounded-t-[19px] border-b border-border/65 bg-muted/20">
              <ComposerPlanFollowUpBanner
                ...
                planTitle={proposedPlanTitle(props.activeProposedPlan.planMarkdown) ?? null}
              />
            </div>
          ) : null}
          ...
          >
            ...
            {voiceSupported && speech.isListening && speech.interimTranscript ? (
                ...
                <div className="mb-3 space-y-2">
                  {props.selectedProvider === "claude-code" && (
                    <div className="flex items-center gap-2 rounded-md border border-warning/40 bg-warning/10 px-2.5 py-1.5 text-xs text-warning">
                      ...
                      <span>Image attachments are not sent for Claude Code.</span>
                    </div>
                  )}
                ...
                {props.canAttachImages && props.onAttachImages && (
                  <>
                    ...
                    <Tooltip>
                      ...
                      />
                      <TooltipPopup side="top">Attach images to this message</TooltipPopup>
                    </Tooltip>
                ...
                <Tooltip>
                  ...
                  <TooltipPopup side="top" className="max-w-72 whitespace-normal">
                    {props.selectedProvider === "gemini" ? (
                      <p>
                        Gemini usage is metered and can be temporarily throttled after sustained
                        heavy use (for example, hitting 5‑hour activity windows or weekly limits).
                        If turns stay stuck on “Working”, you may need to wait before sending more
                        requests.
                      </p>
                    ) : props.selectedProvider === "claude-code" ? (
                      <p>
                        Claude Code usage is subject to your account&apos;s limits and safety
                        policies. Long‑running “Working” states usually mean the provider is busy or
                        enforcing rate limits.
                      </p>
                    ) : (
                      <p>
                        Codex usage draws from your OpenAI quota and may be slowed or throttled if
                        you approach configured limits or send many large turns in a short period.
                      </p>
                    )}
                  </TooltipPopup>
                </Tooltip>
                ...
                <Button
                  ...
                  onClick={() =>
                    void props.handleRuntimeModeChange(
                      props.runtimeMode === "full-access" ? "approval-required" : "full-access",
                    )
                  }
                  title={
                        ...
                        render={
                          <Button
                            ...
                            className={cn(
                              ...
                              speech.status === "starting" || speech.status === "listening"
                          ...
                          >
                            {speech.status === "starting" || speech.status === "listening" ? (
                              <span className="flex items-center gap-1.5">
                                ...
                                />
                                <span className="sr-only sm:not-sr-only">
                                  {speech.status === "starting" ? "Starting…" : "Listening…"}
                                </span>
                              </span>
                            ) : (
