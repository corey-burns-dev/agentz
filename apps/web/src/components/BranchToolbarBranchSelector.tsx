...
export function BranchToolbarBranchSelector({
	...
	const canCreateBranch =
		effectiveEnvMode === "local" && trimmedBranchQuery.length > 0;
	...
	const selectBranch = (branch: GitBranch) => {
		...
		if (branch.worktreePath) {
			...
			onSetThreadBranch(
				branch.name,
				isMainWorktree ? null : branch.worktreePath,
			);
			setIsBranchMenuOpen(false);
			onComposerFocusRequest?.();
			return;
	...
	}, [branchListVirtualizer, isBranchMenuOpen]);
					...
					>
						{virtualBranchRows.map((virtualRow) => {
							...
							if (
								...
								itemValue === createBranchItemValue
							) {
								return (
									<ComboboxItem
										hideIndicator
									...
									>
										<span className="truncate">
											Create new branch "{trimmedBranchQuery}"
										</span>
									</ComboboxItem>
								);
							}
							...
							return (
								...
								>
									<div className="flex w-full items-center justify-between gap-2">
										...
										{badge && (
											<span className="shrink-0 text-2xs text-muted-foreground/45">
												{badge}
											</span>
										)}
									</div>
