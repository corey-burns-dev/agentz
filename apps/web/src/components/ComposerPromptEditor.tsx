...
function renderMentionChipDom(container: HTMLElement, pathValue: string): void {
	...
	icon.src = getVscodeIconUrlForEntry(
		pathValue,
		inferMentionPathKind(pathValue),
		theme,
	);
			...
			(event) => {
				...
				if ($isTextNode(anchorNode)) {
					...
					if ($isElementNode(parent)) {
						...
						if (
							index > 0 &&
							removeMentionNode(parent.getChildAtIndex(index - 1))
						) {
							return true;
						}
					}
				...
				if ($isElementNode(anchorNode)) {
					...
					if (
						childIndex >= 0 &&
						removeMentionNode(anchorNode.getChildAtIndex(childIndex))
					) {
						return true;
					}
				}
...
>(function ComposerPromptEditor(
	...
	const initialConfig = useMemo<InitialConfigType>(
		() => ({
			namespace: "agents-composer-editor",
			editable: true,
