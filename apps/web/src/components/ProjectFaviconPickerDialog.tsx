...
export function ProjectFaviconPickerDialog({
	...
	const candidates = useMemo(() => {
		return (data?.entries ?? [])
			.filter(
				(entry) => entry.kind === "file" && isProjectImageFilePath(entry.path),
			)
			.toSorted((left, right) =>
				compareProjectFaviconPaths(left.path, right.path),
			);
	}, [data?.entries]);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogPopup className="max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						Search this repo for an icon file. Try <code>favicon</code>,{" "}
						<code>icon</code>, <code>logo</code>, or a path fragment.
					</DialogDescription>
				</DialogHeader>
				<DialogPanel className="space-y-4">
					<div className="space-y-2">
						...
						<p className="text-xs text-muted-foreground">
							Selecting a file stores a local override for this project on this
							device.
						</p>
					</div>
