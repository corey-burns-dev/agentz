...
export default function GitActionsControl({
	...
	const initMutation = useMutation(
		gitInitMutationOptions({ cwd: gitCwd, queryClient }),
	);
	...
	const pullMutation = useMutation(
		gitPullMutationOptions({ cwd: gitCwd, queryClient }),
	);

	const isRunStackedActionRunning =
		useIsMutating({ mutationKey: gitMutationKeys.runStackedAction(gitCwd) }) >
		0;
	const isPullRunning =
		useIsMutating({ mutationKey: gitMutationKeys.pull(gitCwd) }) > 0;
	const isGitActionRunning = isRunStackedActionRunning || isPullRunning;
	const isDefaultBranch = useMemo(() => {
		const branchName = gitStatusForActions?.branch;
		if (!branchName) return false;
		const current = branchList?.branches.find(
			(branch) => branch.name === branchName,
		);
		return (
			current?.isDefault ?? (branchName === "main" || branchName === "master")
		);
	...
	const runGitActionWithToast = useCallback(
		async ({
			...
			const pushTarget =
				!featureBranch && actionBranch ? `origin/${actionBranch}` : undefined;
	...
	}, [dialogCommitMessage, isCommitDialogOpen, runGitActionWithToast]);
					...
					<DialogPanel className="space-y-4">
						<div className="space-y-3 rounded-lg border border-input bg-muted/40 p-3 text-xs">
							...
							<div className="space-y-1">
								...
								{!gitStatusForActions ||
								gitStatusForActions.workingTree.files.length === 0 ? (
