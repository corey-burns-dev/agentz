...
export const DIFF_SIZE_OPTIONS = ["compact", "balanced", "comfortable"] as const;
...
export const PROJECT_FAVICON_SIZE_OPTIONS = ["small", "medium", "large"] as const;
export type ProjectFaviconDisplaySize = (typeof PROJECT_FAVICON_SIZE_OPTIONS)[number];

export const SIDEBAR_SPACING_OPTIONS = ["compact", "default", "spacious"] as const;
export type SidebarSpacingOption = (typeof SIDEBAR_SPACING_OPTIONS)[number];

export type ResolvedTheme = "light" | "dark";

const LEGACY_THEME_PRESETS = [...DARK_THEME_PRESETS, LIGHT_THEME_PRESET] as const;
...
const UISettingsSchema = Schema.Struct({
  ...
  themePreset: Schema.Literals(DARK_THEME_PRESETS).pipe(
    Schema.withConstructorDefault(() => Option.some<ThemePreset>("default-dark")),
  ),
  glassEffect: Schema.Boolean.pipe(Schema.withConstructorDefault(() => Option.some(false))),
  fontSize: Schema.Number.check(Schema.isBetween({ minimum: 75, maximum: 125 })).pipe(
    Schema.withConstructorDefault(() => Option.some(100)),
  ),
  density: Schema.Literals(DENSITY_OPTIONS).pipe(
    Schema.withConstructorDefault(() => Option.some<DensityOption>("comfortable")),
  ),
  diffSize: Schema.Literals(DIFF_SIZE_OPTIONS).pipe(
    Schema.withConstructorDefault(() => Option.some<DiffSizeOption>("balanced")),
  ),
  diffWrap: Schema.Boolean.pipe(Schema.withConstructorDefault(() => Option.some(false))),
  diffShowLineNumbers: Schema.Boolean.pipe(Schema.withConstructorDefault(() => Option.some(true))),
  ...
  projectFaviconSize: Schema.Literals(PROJECT_FAVICON_SIZE_OPTIONS).pipe(
    Schema.withConstructorDefault(() => Option.some<ProjectFaviconDisplaySize>("medium")),
  ),
  sidebarSpacing: Schema.Literals(SIDEBAR_SPACING_OPTIONS).pipe(
    Schema.withConstructorDefault(() => Option.some<SidebarSpacingOption>("default")),
  ),
});
...
function parseLegacyPreset(value: unknown): (typeof LEGACY_THEME_PRESETS)[number] | null {
  return LEGACY_THEME_PRESETS.includes(value as (typeof LEGACY_THEME_PRESETS)[number])
    ? (value as (typeof LEGACY_THEME_PRESETS)[number])
    : null;
}

function parseThemeMode(value: unknown): ThemeMode | null {
  return THEME_MODE_OPTIONS.includes(value as ThemeMode) ? (value as ThemeMode) : null;
}

function parseThemePreset(value: unknown): ThemePreset | null {
  return DARK_THEME_PRESETS.includes(value as ThemePreset) ? (value as ThemePreset) : null;
}
...
export function resolveThemeMode(themeMode: ThemeMode, systemDark: boolean): ResolvedTheme {
  if (themeMode === "system") {
...
export function resolveUISettingsTheme(
  ...
  return {
    appliedThemePreset: resolvedTheme === "light" ? LIGHT_THEME_PRESET : settings.themePreset,
    resolvedTheme,
  };
}
...
function applyResolvedThemeClass(resolvedTheme: ResolvedTheme, suppressTransitions: boolean): void {
...
function handleStorageChange(event: StorageEvent) {
  if (event.key === UI_SETTINGS_STORAGE_KEY || event.key === LEGACY_THEME_STORAGE_KEY) {
    const settings = getUISettingsSnapshot();
...
export function applyUISettings(settings: UISettings, suppressTransitions = false): void {
  if (typeof document === "undefined") return;

  const html = document.documentElement;
  const { resolvedTheme, appliedThemePreset } = resolveUISettingsTheme(settings);
