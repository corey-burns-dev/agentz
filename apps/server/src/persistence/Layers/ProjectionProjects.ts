import { ProjectScript } from "@agentz/contracts";
...
const makeProjectionProjectRepository = Effect.gen(function* () {
		...
		getProjectionProjectRow(input).pipe(
			...
			Effect.flatMap((rowOption) =>
				Option.match(rowOption, {
					...
					onSome: (row) =>
						Effect.succeed(
							Option.some(row as Schema.Schema.Type<typeof ProjectionProject>),
						),
				}),
			),
		);
	...
	const listAll: ProjectionProjectRepositoryShape["listAll"] = () =>
		listProjectionProjectRows().pipe(
			...
			Effect.map(
				(rows) =>
					rows as ReadonlyArray<Schema.Schema.Type<typeof ProjectionProject>>,
			),
		);
