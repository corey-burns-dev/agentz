...
const faviconResolutionCache = new Map<string, { resolvedAt: number; filePath: string | null }>();
...
async function buildAppRoots(projectCwd: string): Promise<string[]> {
  const roots: string[] = [
    projectCwd,
    ...NESTED_APP_DIRECTORIES.map((directory) => path.join(projectCwd, directory)),
  ];
...
function resolveIconHref(
  ...
  if (cleanHref.length === 0 || cleanHref.startsWith("data:") || /^[a-z]+:/i.test(cleanHref)) {
    return [];
  }
  ...
  const candidates = cleanHref.startsWith("/")
    ? [path.join(appRoot, "public", normalizedHref), path.join(appRoot, normalizedHref)]
    : [
        fromSourceDir,
  ...
  return [...new Set(candidates)].filter((candidate) => isPathWithinProject(projectCwd, candidate));
}
...
async function findWellKnownProjectFavicon(projectCwd: string): Promise<string | null> {
  for (const appRoot of await buildAppRoots(projectCwd)) {
    for (const candidate of FAVICON_CANDIDATES) {
      const resolved = await resolveExistingFile(projectCwd, path.join(appRoot, candidate));
    ...
    for (const sourceFileRelativePath of ICON_SOURCE_FILES) {
      ...
      for (const resolvedPath of resolveIconHref(projectCwd, appRoot, sourceFile, href)) {
...
async function findRecursiveProjectFavicon(projectCwd: string): Promise<string | null> {
  const pendingDirectories = [projectCwd];
  let bestLikelyMatch: { relativePath: string; absolutePath: string } | null = null;
  let bestImageMatch: { relativePath: string; absolutePath: string } | null = null;
  ...
  while (pendingDirectories.length > 0) {
    ...
    for (const dirent of dirents) {
      ...
      const relativePath = normalizeProjectRelativePath(path.relative(projectCwd, absolutePath));
      ...
      if (
        bestImageMatch === null ||
        compareProjectFaviconPaths(relativePath, bestImageMatch.relativePath) < 0
      ) {
      ...
      if (
        ...
        compareProjectFaviconPaths(relativePath, bestLikelyMatch.relativePath) < 0
      ) {
...
async function resolveProjectFavicon(projectCwd: string): Promise<string | null> {
...
async function handleProjectFaviconRequest(url: URL, res: http.ServerResponse): Promise<void> {
  ...
  if (requestedRelativePath) {
    ...
    const resolved = await resolveExistingFile(projectCwd, path.join(projectCwd, normalizedPath));
    if (!resolved) {
