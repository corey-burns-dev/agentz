...
function parseKeybindingWhenExpression(
	expression: string,
...
export const ResolvedKeybindingsFromConfig = Schema.Array(
...
const DEFAULT_RESOLVED_KEYBINDINGS =
	compileResolvedKeybindingsConfig(DEFAULT_KEYBINDINGS);

const RawKeybindingsEntries = Schema.fromJsonString(
...
const makeKeybindings = Effect.gen(function* () {
		...
		> {
			...
			return yield* Effect.forEach(rawConfig, (entry) =>
				Effect.gen(function* () {
					...
					const resolved = Schema.decodeExit(ResolvedKeybindingFromConfig)(
						decodedRule.value,
					);
			...
			).pipe(Effect.map(EffectArray.filter(Predicate.isNotNull)));
		},
	);
	...
	const loadRuntimeCustomKeybindingsConfig = Effect.fn(
		...
		> {
			...
			const decodedEntries = Schema.decodeUnknownExit(RawKeybindingsEntries)(
				rawConfig,
			);
			...
			for (const [index, entry] of decodedEntries.value.entries()) {
				...
				const resolvedRule = Schema.decodeExit(ResolvedKeybindingFromConfig)(
					decodedRule.value,
				);
	...
	yield* Stream.runForEach(fs.watch(keybindingsConfigDir), (event) => {
		...
		return revalidateAndEmit.pipe(
			Effect.catch((error) =>
				Effect.logWarning(
					"failed to revalidate keybindings config after file update",
					{
						path: keybindingsConfigPath,
						detail: error.detail,
						cause: error.cause,
