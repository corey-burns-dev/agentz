...
const makeProviderService = (options?: ProviderServiceLiveOptions) =>
	Effect.gen(function* () {
		...
		const respondToRequest: ProviderServiceShape["respondToRequest"] = (
			rawInput,
		) =>
			Effect.gen(function* () {
				const input = yield* decodeInputOrValidationError({
		...
		const respondToUserInput: ProviderServiceShape["respondToUserInput"] = (
			rawInput,
		) =>
			Effect.gen(function* () {
				const input = yield* decodeInputOrValidationError({
		...
		const listSessions: ProviderServiceShape["listSessions"] = () =>
			Effect.gen(function* () {
				...
				const activeSessions = sessionsByProvider.flat();
				const persistedBindings = yield* directory.listThreadIds().pipe(
					...
					Effect.orElseSucceed(
						() => [] as Array<Option.Option<ProviderRuntimeBinding>>,
					),
				);
