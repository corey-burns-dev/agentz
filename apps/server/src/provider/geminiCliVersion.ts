const GEMINI_VERSION_PATTERN = /\bv?(\d+\.\d+(?:\.\d+)?(?:-[0-9A-Za-z.-]+)?)\b/;

export const MINIMUM_GEMINI_CLI_VERSION = "0.37.0";
...
function normalizeGeminiVersion(version: string): string {
...
function parseSemver(version: string): ParsedSemver | null {
  const normalized = normalizeGeminiVersion(version);
...
export function compareGeminiCliVersions(left: string, right: string): number {
...
export function parseGeminiCliVersion(output: string): string | null {
  const match = GEMINI_VERSION_PATTERN.exec(output);
  if (!match?.[1]) {
    return null;
  }
  ...
  return normalizeGeminiVersion(match[1]);
}

export function isGeminiCliVersionSupported(_version: string): boolean {
  return true; // No specific minimum version requirement for Gemini currently
}
