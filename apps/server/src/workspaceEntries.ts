...
async function filterGitIgnoredPaths(
	...
	for (const relativePath of relativePaths) {
		...
		if (
			chunkBytes >= GIT_CHECK_IGNORE_MAX_STDIN_BYTES &&
			!(await flushChunk())
		) {
			return relativePaths;
		}
...
async function buildWorkspaceIndexFromGit(
	...
	return {
		...
		truncated:
			Boolean(listedFiles.stdoutTruncated) ||
			entries.length > WORKSPACE_INDEX_MAX_ENTRIES,
	};
}
...
async function buildWorkspaceIndex(cwd: string): Promise<WorkspaceIndex> {
	...
	while (pendingDirectories.length > 0 && !truncated) {
		...
		const candidateEntriesByDirectory = directoryEntries.map(
			(directoryEntry) => {
		...
		const candidatePaths = candidateEntriesByDirectory.flatMap(
			(candidateEntries) => candidateEntries.map((entry) => entry.relativePath),
		);
...
export async function searchWorkspaceEntries(
	...
	const ranked = candidates.toSorted((left, right) => {
		const scoreDelta =
			scoreEntry(left, normalizedQuery) - scoreEntry(right, normalizedQuery);
