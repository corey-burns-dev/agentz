...
export class ClaudeCodeAppServerManager extends EventEmitter<ClaudeCodeAppServerManagerEvents> {
		...
		try {
			...
			assertSupportedClaudeCodeCliVersion({
				...
				...(claudeCodeOptions.homePath
					? { homePath: claudeCodeOptions.homePath }
					: {}),
			});
	...
	async sendTurn(
		...
		// Build args
		const useControlProtocol =
			context.session.runtimeMode === "approval-required";
		const resolvedModel =
			normalizeClaudeCodeModelSlug(input.model) ?? context.session.model;
	...
	async rollbackThread(
		threadId: ThreadId,
		_numTurns: number,
	): Promise<ClaudeCodeThreadSnapshot> {
	...
	async respondToRequest(
		...
		const response = isApproved
			...
			: buildControlBlockResponse(
					pendingRequest.claudeRequestId,
					"Permission denied.",
				);
	...
	async respondToUserInput(
		...
		this.requireSession(threadId);
		throw new Error(
			"Claude Code sessions do not support structured user input responses.",
		);
	}
	...
	private handleControlRequest(
		...
		const itemId =
			existingItem?.itemId ?? ProviderItemId.makeUnsafe(randomUUID());
	...
	private finishTurn(
		...
		for (const itemId of activeTurn.startedItemIds) {
			const toolCtx = Array.from(activeTurn.toolItems.values()).find(
				(t) => t.itemId === itemId,
			);
