...
import { Effect, Exit, Layer, type PlatformError, PubSub, Scope, Stream } from "effect";
import type { SqlClient, SqlError } from "effect/unstable/sql";
import { afterEach, describe, expect, it, vi } from "vitest";
import WebSocket from "ws";
import { ServerConfig, type ServerConfigShape } from "./config";
import { GitCommandError, GitManagerError, type GitManagerServiceError } from "./git/Errors.ts";
...
describe("WebSocket Server", () => {
  ...
  it("catches websocket message handler rejections and keeps the socket usable", async () => {
    ...
    const brokenOpenService: OpenShape = {
      openBrowser: () => Effect.void,
      openInEditor: () =>
        Effect.sync(() => BigInt(1)).pipe(Effect.map((result) => result as unknown as undefined)),
    };
  ...
  it("supports git.status over websocket", async () => {
    ...
    const runStackedAction = vi.fn(
      () =>
        Effect.void as unknown as Effect.Effect<GitRunStackedActionResult, GitManagerServiceError>,
    );
  ...
  it("returns errors from git.runStackedAction", async () => {
    ...
    const gitManager: GitManagerShape = {
      ...
      status: vi.fn(
        () => Effect.void as unknown as Effect.Effect<GitStatusResult, GitManagerServiceError>,
      ),
      runStackedAction,
    };
