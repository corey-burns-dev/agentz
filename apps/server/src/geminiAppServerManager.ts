...
function runtimeModeToSessionMode(
...
export class GeminiAppServerManager extends EventEmitter<GeminiAppServerManagerEvents> {
	...
	async startSession(
		input: GeminiAppServerStartSessionInput,
		...
		try {
			...
			this.updateSession(context, {
				...
				...(context.acpSessionId
					? { resumeCursor: { threadId: context.acpSessionId } }
					: {}),
			});
	...
	async interruptTurn(threadId: ThreadId, turnId?: TurnId): Promise<void> {
		...
		const activeTurnId =
			context.activeTurn?.turnId ?? context.session.activeTurnId;
	...
	async respondToRequest(
		...
		const allowOnce = pendingRequest.options?.find(
			(option) => option.kind === "allow_once",
		);
		const allowAlways = pendingRequest.options?.find(
			(option) => option.kind === "allow_always",
		);
		const rejectOnce = pendingRequest.options?.find(
	...
	private handlePromptFailed(
		...
		const message =
			error instanceof Error ? error.message : "Gemini prompt failed.";
	...
	private handleServerNotification(
		...
		if (sessionUpdate === "tool_call" || sessionUpdate === "tool_call_update") {
			...
			const detail =
				textFromToolContent(toolCall.content) ??
				existingItem?.detail ??
	...
	private handleServerRequest(
		...
		const toolCallId = asString(toolCall?.toolCallId);
		const existingItem = toolCallId
			? activeTurn.toolItems.get(toolCallId)
			: undefined;
		const itemId =
			existingItem?.id ??
			ProviderItemId.makeUnsafe(asString(toolCall?.toolCallId) ?? randomUUID());
		if (toolCallId && !existingItem) {
			...
			const item: GeminiTurnItemSnapshot = {
				...
				toolCallId,
				...(toolCall?.content
					? { content: toolCall.content as GeminiAcpToolContent[] }
					: {}),
			};
			activeTurn.toolItems.set(toolCallId, item);
			this.trackTurnItem(context, item);
