...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const previousStdinMustContain = process.env.AGENTS_FAKE_CODEX_STDIN_MUST_CONTAIN;
      const previousStdinMustNotContain = process.env.AGENTS_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;

      yield* Effect.sync(() => {
        process.env.PATH = `${binDir}:${previousPath ?? ""}`;
        process.env.AGENTS_FAKE_CODEX_OUTPUT_B64 = Buffer.from(input.output, "utf8").toString(
          "base64",
        );
        ...
        if (input.stdinMustContain !== undefined) {
          process.env.AGENTS_FAKE_CODEX_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.AGENTS_FAKE_CODEX_STDIN_MUST_CONTAIN;
        }

        if (input.stdinMustNotContain !== undefined) {
          process.env.AGENTS_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN = input.stdinMustNotContain;
        } else {
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousRequireImage === undefined) {
          ...
          process.env.AGENTS_FAKE_CODEX_REQUIRE_IMAGE = previous.previousRequireImage;
        }

        if (previous.previousStdinMustContain === undefined) {
          delete process.env.AGENTS_FAKE_CODEX_STDIN_MUST_CONTAIN;
        } else {
          process.env.AGENTS_FAKE_CODEX_STDIN_MUST_CONTAIN = previous.previousStdinMustContain;
        }
