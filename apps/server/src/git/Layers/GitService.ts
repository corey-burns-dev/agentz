...
const makeGitService = Effect.gen(function* () {
	...
	const execute: GitServiceShape["execute"] = Effect.fnUntraced(
		function* (input) {
			const commandInput = {
				...input,
				args: [...input.args],
			...
			const commandEffect = Effect.gen(function* () {
				const child = yield* commandSpawner
					...
					.pipe(
						Effect.mapError(
							toGitCommandError(commandInput, "failed to spawn."),
						),
					);
				...
				const [stdout, stderr, exitCode] = yield* Effect.all(
					[
						...
						child.exitCode.pipe(
							...
							Effect.mapError(
								toGitCommandError(commandInput, "failed to report exit code."),
							),
						),
					],
