...
function createGitHubCliWithFakeGh(scenario: FakeGhScenario = {}): {
  ...
  const toArgsWithRepository = (args: ReadonlyArray<string>, repository?: string): string[] =>
    repository && repository.trim().length > 0 ? [...args, "--repo", repository] : [...args];
  ...
  const execute: GitHubCliShape["execute"] = (input) => {
    ...
    if (args[0] === "pr" && args[1] === "create") {
      return Effect.succeed({
        stdout: `${scenario.createdPrUrl ?? "https://github.com/burnsco/agents/pull/101"}\n`,
        stderr: "",
        code: 0,
        signal: null,
        timedOut: false,
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("status resolves the GitHub repository from origin before calling gh", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("agents-git-manager-");
      yield* initRepo(repoDir);
  ...
  it.effect("listIssues returns an empty list when the repository has issues disabled", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("agents-git-manager-");
      yield* initRepo(repoDir);
      yield* runGit(repoDir, [
