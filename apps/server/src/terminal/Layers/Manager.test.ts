...
import { DEFAULT_TERMINAL_ID, type TerminalEvent, type TerminalOpenInput } from "@agents/contracts";
  ...
  it("retries with fallback shells when preferred shell spawn fails", async () => {
    ...
    if (process.platform === "win32") {
      ...
      if (process.env.SHELL) {
        expectedShells.push(process.env.SHELL);
      }
      expect(ptyAdapter.spawnInputs.some((input) => expectedShells.includes(input.shell))).toBe(
        true,
      );
    }
