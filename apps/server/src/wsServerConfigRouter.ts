import type { EditorId, ServerProviderStatus, WebSocketRequest } from "@agents/contracts";
...
export function createServerConfigRouter(context: ServerConfigRouterContext): WsRouteHandler {
  return (request: WebSocketRequest) => {
    switch (request.body._tag) {
      case WS_METHODS.serverGetConfig:
        return Effect.gen(function* () {
          const keybindingsConfig = yield* context.keybindingsManager.loadConfigState;
          return {
            cwd: context.cwd,
      ...
      case WS_METHODS.serverUpsertKeybinding:
        return Effect.gen(function* () {
          ...
          const keybindingsConfig = yield* context.keybindingsManager.upsertKeybindingRule(body);
