...
function processResult(
	overrides: Partial<ProcessRunResult> &
		Pick<ProcessRunResult, "stdout" | "code">,
): ProcessRunResult {
	return {
...
describe("searchWorkspaceEntries git-ignore chunking", () => {
	...
	it("chunks git check-ignore stdin to avoid building giant strings", async () => {
		const ignoredPaths = Array.from(
			{ length: 5000 },
			(_, index) =>
				`ignored/${index.toString().padStart(5, "0")}/${"x".repeat(80)}.ts`,
		);
		...
		runProcessMock.mockImplementation(async (_command, args, options) => {
			...
			if (args[0] === "check-ignore") {
				checkIgnoreCalls += 1;
				const chunkPaths = (options?.stdin ?? "")
					.split("\0")
					.filter((value) => value.length > 0);
				const chunkIgnored = chunkPaths.filter((value) =>
					value.startsWith("ignored/"),
				);
				return processResult({
		...
		assert.isFalse(
			result.entries.some((entry) => entry.path.startsWith("ignored/")),
		);
