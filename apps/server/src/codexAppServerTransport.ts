...
export function routeCodexProtocolLine(line: string, handlers: CodexProtocolLineHandlers): void {
  let parsed: unknown;
  try {
    parsed = JSON.parse(line);
  } catch {
    handlers.onProtocolError("protocol/parseError", "Received invalid JSON from codex app-server.");
    return;
  }
...
export function writeJsonRpcMessage(context: CodexSessionContext, message: unknown): void {
