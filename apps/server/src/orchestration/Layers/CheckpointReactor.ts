...
const make = Effect.gen(function* () {
	...
	const captureCheckpointFromTurnCompletion = Effect.fnUntraced(function* (
		...
		if (!checkpointCwd) {
			yield* Effect.logWarning(
				"checkpoint capture skipped: no active provider session cwd",
				{
					threadId: thread.id,
					turnId,
				},
		...
		if (!isGitWorkspace(checkpointCwd)) {
			yield* Effect.logDebug(
				"checkpoint capture skipped for non-git workspace",
				{
					threadId: thread.id,
					turnId,
					cwd: checkpointCwd,
		...
		if (!fromCheckpointExists) {
			yield* Effect.logWarning(
				"checkpoint completion missing pre-turn baseline",
				{
					threadId: thread.id,
					turnId,
					fromTurnCount,
	...
	const processDomainEvent = Effect.fnUntraced(function* (
	...
	const processRuntimeEvent = Effect.fnUntraced(function* (
	...
	const start: CheckpointReactorShape["start"] = Effect.gen(function* () {
		...
		yield* Effect.forkScoped(
			Effect.forever(
				Queue.take(queue).pipe(Effect.flatMap(processInputSafely)),
			),
		);
		...
		yield* Effect.forkScoped(
			Stream.runForEach(orchestrationEngine.streamDomainEvents, (event) => {
				...
				return Queue.offer(queue, { source: "domain", event }).pipe(
					Effect.asVoid,
				);
			}),
		);
		...
		yield* Effect.forkScoped(
			Stream.runForEach(providerService.streamEvents, (event) => {
				...
				return Queue.offer(queue, { source: "runtime", event }).pipe(
					Effect.asVoid,
				);
			}),
		);
