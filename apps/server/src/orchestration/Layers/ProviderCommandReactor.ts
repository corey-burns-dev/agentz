...
const make = Effect.gen(function* () {
  ...
  const sendTurnForThread = Effect.fnUntraced(function* (input: {
    ...
    const waitForSendableSession = Effect.fnUntraced(function* () {
      ...
      return yield* Effect.fail(new Error("Provider session did not become ready in time."));
    });
    ...
    if (activeSession.status === "error") {
      return yield* Effect.fail(
        new Error(
          activeSession.lastError ?? "Provider session reported an error before the turn started.",
        ),
      );
    }
    if (activeSession.status === "closed") {
      return yield* Effect.fail(new Error("Provider session closed before the turn started."));
    }
    const sessionModelSwitch =
