...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: { readonly stateDir?: string }) {
    ...
    const stateDir = input?.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "agents-reactor-"));
  ...
  it("restarts Gemini sessions as Gemini when runtime mode changes", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.makeUnsafe("cmd-runtime-mode-set-gemini-initial"),
        threadId: ThreadId.makeUnsafe("thread-1"),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-gemini-1"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-gemini-1"),
          role: "user",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.runtime-mode.set",
        commandId: CommandId.makeUnsafe("cmd-runtime-mode-set-gemini-next"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        runtimeMode: "approval-required",
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-gemini-2"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-gemini-2"),
          role: "user",
