					...
					(entry) =>
						Effect.gen(function* () {
							...
							yield* fileSystem.remove(
								path.join(attachmentsRootDir, normalizedEntry),
								{
									force: true,
								},
							);
...
const makeOrchestrationProjectionPipeline = Effect.gen(function* () {
	...
	const applyProjectsProjection: ProjectorDefinition["apply"] = (
		event,
	...
	const applyThreadsProjection: ProjectorDefinition["apply"] = (
		event,
				...
				case "thread.message-sent": {
					...
					if (Option.isSome(existingTurn)) {
						yield* projectionTurnRepository.upsertByTurnId({
							...
							startedAt:
								existingTurn.value.startedAt ?? event.payload.createdAt,
							requestedAt:
