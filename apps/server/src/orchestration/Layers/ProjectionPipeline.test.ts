...
const runWithProjectionPipelineLayer = <A, E>(
	...
	Effect.acquireUseRelease(
		Effect.sync(() =>
			ManagedRuntime.make(makeProjectionPipelineTestLayer(stateDir)),
		),
...
projectionLayer("OrchestrationProjectionPipeline", (it) => {
	...
	it.effect(
		...
		() =>
			Effect.sync(() =>
				fs.mkdtempSync(path.join(os.tmpdir(), "t3-projection-attachments-")),
			).pipe(
	...
	it.effect(
		...
		() =>
			Effect.sync(() =>
				fs.mkdtempSync(
					path.join(os.tmpdir(), "t3-projection-attachments-revert-"),
				),
			).pipe(
				Effect.flatMap((stateDir) =>
					Effect.gen(function* () {
						...
						yield* appendAndProject({
							...
							payload: {
								...
								checkpointRef: CheckpointRef.makeUnsafe(
									"refs/agentz/checkpoints/thread-revert-files/turn/1",
								),
								status: "ready",
								files: [],
						...
						yield* appendAndProject({
							...
							payload: {
								...
								checkpointRef: CheckpointRef.makeUnsafe(
									"refs/agentz/checkpoints/thread-revert-files/turn/2",
								),
								status: "ready",
								files: [],
	...
	it.effect("removes thread attachment directory when thread is deleted", () =>
		Effect.sync(() =>
			fs.mkdtempSync(
				path.join(os.tmpdir(), "t3-projection-attachments-delete-"),
			),
		).pipe(
			Effect.flatMap((stateDir) =>
				Effect.gen(function* () {
	...
	it.effect("ignores unsafe thread ids for attachment cleanup paths", () =>
		Effect.sync(() =>
			fs.mkdtempSync(
				path.join(os.tmpdir(), "t3-projection-attachments-unsafe-"),
			),
		).pipe(
			Effect.flatMap((stateDir) =>
				Effect.gen(function* () {
					...
					const stateDirSentinelPath = path.join(
						stateDir,
						"state-sentinel.txt",
					);
			...
			Effect.gen(function* () {
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 1,
						checkpointRef: CheckpointRef.makeUnsafe(
							"refs/agentz/checkpoints/thread-conflict/turn/1",
						),
						status: "ready",
						files: [],
			...
			Effect.gen(function* () {
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 1,
						checkpointRef: CheckpointRef.makeUnsafe(
							"refs/agentz/checkpoints/thread-revert/turn/1",
						),
						status: "ready",
						files: [],
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 2,
						checkpointRef: CheckpointRef.makeUnsafe(
							"refs/agentz/checkpoints/thread-revert/turn/2",
						),
						status: "ready",
						files: [],
	...
	() =>
		Effect.gen(function* () {
			const tempDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "t3-projection-pipeline-restart-"),
			);
