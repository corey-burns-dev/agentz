...
const resolveBuildOptions = Effect.fn("resolveBuildOptions")(function* (
	...
	const arch = mergeOptions(
		input.arch,
		Option.fromNullable(env.arch),
		getDefaultArch(platform),
	);
	const version = mergeOptions(
		input.buildVersion,
		Option.fromNullable(env.version),
		undefined,
	);
	const outputDir = path.resolve(
		...
		mergeOptions(
			input.outputDir,
			Option.fromNullable(env.outputDir),
			"release",
		),
	);
