...
async function loadLanguagesCollection() {
	...
	if (!response.ok) {
		throw new Error(
			`Failed to fetch languages.ts: ${response.status} ${response.statusText}`,
		);
	}
...
function buildLanguageAssociations(manifest, languages) {
	...
	for (const entry of Object.values(languages)) {
		...
		const knownExtensions = Array.isArray(entry.knownExtensions)
			? entry.knownExtensions
			: [];
		const knownFilenames = Array.isArray(entry.knownFilenames)
			? entry.knownFilenames
			: [];
