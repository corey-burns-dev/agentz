...
> = {
	...
	"dev:desktop": {
		...
		workspaces: [
			...
			{ cwd: path.join(ROOT, "apps/desktop"), script: "dev" },
			{ cwd: path.join(ROOT, "apps/web"), script: "dev" },
		],
	},
};
...
const devRunnerCli = Command.make("dev-runner", {
	...
	dryRun: Flag.boolean("dry-run").pipe(
		Flag.withDescription(
			"Resolve mode/ports/env and print, but do not spawn dev processes.",
		),
		Flag.withDefault(false),
	),
	extraArgs: Argument.string("extra-arg").pipe(
		Argument.withDescription(
			"Extra args for the server dev script (pass after `--`).",
		),
		Argument.variadic(),
	),
}).pipe(
